...
- (instancetype)initWithX:(float)x y:(float)y title:(NSString*)title{
    return [self initWithX:x y:y title:title date:[NSDate date]];
}
...
- (BOOL)isEqualToPoint:(SmartPoint *)point
{
   ...
   if (point == nil)
      return NO;
   if (fabs(self.x - point.x)>0.0000001)//поменять на правильный
      return NO;
...
- (NSString *)description
{
   ...
   [description appendFormat:@"self.x=%g", self.x];
